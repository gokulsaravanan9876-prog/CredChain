...
def _register_verifier(client, email, name):
    resp = client.post(
        "/api/auth/register",
        json={"email": email, "password": "Password123", "full_name": "AI Verifier", "role": "verifier", "company_name": name},
    )
...
def _register_institution(client, email, name):
    resp = client.post(
        "/api/auth/register",
        json={"email": email, "password": "Password123", "full_name": "AI Inst", "role": "institution", "institution_name": name},
    )
...
def test_analyze_real_job_returns_real_company_and_job_data(client, db_session):
    verifier = _register_verifier(client, "ai-co-1@test.credchain.dev", "AI Analysis Co 1")
    inst = _register_institution(client, "ai-inst-1@test.credchain.dev", "AI University 1")
...
def test_analyze_nonexistent_job_404(client, db_session):
    ...
    inst = _register_institution(client, "ai-inst-2@test.credchain.dev", "AI University 2")
...
def test_analyze_job_requires_student_role(client, db_session):
    verifier = _register_verifier(client, "ai-co-3@test.credchain.dev", "AI Analysis Co 3")
    job = client.post(
        "/api/companies/me/jobs",
